			...
			this.connect().delay(2000).then(function(){
				...
				if (!this.socket || !this.socket.writable) {
					...
					if (operation.retry(err)) {
						debug('Retrying connection X1');
						return;
					} else {
						debug('All retries are done, raising error ' + operation.mainError());

						var err = operation.mainError();
						if (err && err.code === Err.CONNECTION) {
				...
				if (err) {
					if (err.code & Err.IS_RECOVERABLE) {
						if (operation.retry(err)) {
							debug('Retrying connection X2');
							return;
						}
					}
				} else {
					if (operation.retry(err)) {
						debug('Retrying connection X3');
						return;
					}
				}
...
APNS.prototype.onSocketClose = function() {
	...
	if (this.connectWithBackoffDeferred && this.connectWithBackoffDeferred.promise.isPending()) {
...
APNS.prototype.abortAll = function (code, name) {
	...
	this.notifications = new FixedSizeDequeue(this.options.apnsNotificationCacheForErrorPurposes);
...
APNS.prototype.send = function (messageId, content, encoding, expiry, device, locale) {
	...
	if (!this.free() || this.closed) {
		debug('Won\'t send message id %j because socket is not free (%d) or closed (%d)', !this.free(), !!this.closed);
		return -1;
	}
